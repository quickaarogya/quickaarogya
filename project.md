...
- **Desktop E-Commerce Storefront Transformation**: Removed the internal admin/dashboard left sidebar from the consumer storefront layout and implemented a top navigation-centric e-commerce header (`DesktopHeader.tsx`) with instant mode switching (`Doctors` | `Pharma` | `Care Hub`), location selector (`⚡ 10 Mins Green Park`), mode-aware category sub-nav strip, prominent search, wishlist count badge, e-commerce cart pill, and consumer profile dropdown.
